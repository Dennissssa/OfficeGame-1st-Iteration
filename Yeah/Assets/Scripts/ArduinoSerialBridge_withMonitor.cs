...
/// </summary>
public class ArduinoSerialBridgeWithMonitor : MonoBehaviour
{
    // ── Configuration ─────────────────────────────────────────
    ...
    [Header("Monitor -> WorkItem Binding")]
    [Tooltip("Drag in the Monitor WorkItem. OnBroken starts computer audio, OnFixed stops it.")]
    [SerializeField] private WorkItem monitorWorkItem;
    ...
    private UnityAction _monitorOnBroken, _monitorOnFixed, _monitorOnBaitingEnded;
    ...
    private void Start()
    {
        ...
        {
            if (ledBindings != null && ledBindings.Length > 0 || phoneWorkItem != null || monitorWorkItem != null || printerWorkItem != null)
                Debug.LogWarning(
        ...
        SendMonitor(MonitorCommand.Normal);
        SendPrinter(false);
    }
    ...
    private void OnDestroy()
    {
        ...
        // Send full hardware reset before closing port so Arduino stops
        // motor, Monitor DFPlayer, phone audio, and LEDs on game exit.
        ResetSystem();
    ...
    {
        if (monitorWorkItem == null) return;

        _monitorOnBroken       = () => SendMonitor(MonitorCommand.Anomaly);
        _monitorOnFixed        = () => SendMonitor(MonitorCommand.Normal);
        _monitorOnBaitingEnded = () => SendMonitor(MonitorCommand.Normal);

        monitorWorkItem.OnBroken.AddListener(_monitorOnBroken);
        monitorWorkItem.OnFixed.AddListener(_monitorOnFixed);
        monitorWorkItem.OnBaitingEnded.AddListener(_monitorOnBaitingEnded);
    }

    private void UnbindMonitor()
    {
        if (monitorWorkItem == null) return;
        if (_monitorOnBroken       != null) monitorWorkItem.OnBroken.RemoveListener(_monitorOnBroken);
        if (_monitorOnFixed        != null) monitorWorkItem.OnFixed.RemoveListener(_monitorOnFixed);
        if (_monitorOnBaitingEnded != null) monitorWorkItem.OnBaitingEnded.RemoveListener(_monitorOnBaitingEnded);
    }
    ...
    /// <summary>Send a monitor/computer audio command: MonitorCommand.Normal / Anomaly.</summary>
    public void SendMonitor(string command)
    {
        SendRaw($"MONITOR:{command}");
    }
    ...
    /// <summary>Monitor/computer audio state strings matching the Arduino protocol.</summary>
